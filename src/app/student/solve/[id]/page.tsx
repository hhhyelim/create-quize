...
function ErrorView({
  ...
  return (
    <PageShell description={description} eyebrow="친구 질문 풀기" title={title}>
...
export default async function SolvePage({ params, searchParams }: SolvePageProps) {
  ...
  const initialQuestionResult = await getRandomQuestion({
  ...
  return (
    <PageShell
      description={`${student.display_name}님, 친구 질문을 읽고 답해 보세요.`}
    ...
    >
      <section className="grid gap-4 lg:grid-cols-[0.8fr_1.2fr]">
        ...
        <Card>
          <SolveQuestionForm
            activityId={activity.id}
            initialQuestion={initialQuestionResult.question}
            studentId={student.id}
          />
        </Card>
      </section>
